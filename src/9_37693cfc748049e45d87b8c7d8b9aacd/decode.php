...
# or a "string"
# and returns the decoded version of
# that input
...
function decode($type, $file) {
	switch ($type) {
		case 'uu':
			$output = convert_uudecode($file);
		break;

		case 'gz':
			$output = gzinflate($file);
		break;

		case 'base64':
			$output = base64_decode($file);
		break;
